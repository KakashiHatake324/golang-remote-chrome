...
func TestBrowserManagerMultipleBrowsers(t *testing.T) {
	...
	opts, err := chrome.NewOptions(&ctx, "", false, "", uuid.New().String(), false, true, []string{}, false, false)
	if err != nil {
		t.Fatalf("Failed to create browser options: %v", err)
	}

	// Create browser options for both browsers (headless mode)
	opts2, err := chrome.NewOptions(&ctx, "", false, "", uuid.New().String(), false, true, []string{}, false, false)
	if err != nil {
		t.Fatalf("Failed to create browser options: %v", err)
	}
